...
export default Ember.Route.extend({
  ...
  afterModel(switches, transition) {
    ...
    return Ember.RSVP.all(ids.map(function(switchId, index) {
      return Ember.$.get(`${config.HOST}/api/stats/flow/${switchId}`, function(payload) {
        var flows = payload[switchId];
        var flowsObjects = flows.map(function(f) {return Ember.Object.create(_that._mapping(f))});
